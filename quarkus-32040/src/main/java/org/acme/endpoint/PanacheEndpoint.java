...
import org.acme.Author;
import org.acme.AuthorRepository;
import org.acme.Book;
import org.acme.BookDescription;
...
public class PanacheEndpoint {
    ...
    @PUT
    @Path("books/{authorId}/{name}")
    public Uni<Response> createBook(Integer authorId, String name) {
        return Panache
                .withTransaction( () -> Panache.getSession()
                        .map( session -> session.getReference( Author.class, authorId ) )
                        .chain( author -> Book.create( author, name ) ) )
